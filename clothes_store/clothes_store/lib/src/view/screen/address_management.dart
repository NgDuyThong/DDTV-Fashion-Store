...
class _AddressManagementScreenState extends State<AddressManagementScreen> {
  ...
  Future<void> _addOrEditAddress({ADDRESS_MODEL? address}) async {
    ...
    if (result == true) {
      if (isEditing) {
        await _editAddress(
            address.addressID!, addressController.text, isDefault);
      } else {
  ...
  Future<void> _editAddress(int id, String address, bool isDefault) async {
    try {
      final result =
          await addressController.updateAddress(id, address, isDefault);
  ...
  Future<void> _setDefaultAddress(int id) async {
    ...
    if (result.statusCode == 200) {
      await _loadAddresses();
      showSnackBar(
          context, "Đặt làm địa chỉ mặc định thành công", Colors.green);
    } else {
      showSnackBar(context, result.message!, Colors.red);
    }
  }

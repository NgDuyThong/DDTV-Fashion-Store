...
class ProductController2 {
  ...
  Future<PRODUCT_LIST_MODEL?> getProducts2(int? pageNumber, String? sortType,
      String? categoryType, int? targetType, String? searchValue) async {
  ...
  Future<List<CATEGORY_MODEL>> getCategories() async {
    ...
    try {
      final response =
          await http.get(Uri.parse('${Server.baseUrl}/api/categories'));
      if (response.statusCode == 200) {
        var data = response.body;
  ...
  Future<PRODUCT_LIST_MODEL?> getRelatedProducts(
      int? categoryType, int? targetType) async {
    // http://localhost:5000/api/products
    // ?page=1&sort=price-asc&category=5&target=1&search=áo%20thun&isActivated=true
    ...
    if (categoryType != null || targetType != null) {
      baseProductsUrl += "?";
      baseProductsUrl += "page=1";
      baseProductsUrl += "&sort=createAt-desc";
      if (categoryType != null) {
        //ID hoặc tên danh mục sản phẩm
        baseProductsUrl += "&category=$categoryType";
      }
    ...
    baseProductsUrl += "&limit=4&isActivated=true";
  ...
  Future<PRODUCT_MODEL?> getProductById(int id) async {
    ...
    try {
      final response = await http
          .get(Uri.parse('${Server.baseUrl}/api/products/flutter/$id'));
      if (response.statusCode == 200) {
        var data = response.body;
        var products = json.decode(data);
        // print(products);
  ...
  Future<STOCK_MODEL?> getStockBySKU(String sku) async {
    ...
    try {
      final response = await http
          .get(Uri.parse('${Server.baseUrl}/api/products/stock/$sku'));
      if (response.statusCode == 200) {
        var data = response.body;

...
class NotificationController {
  Future<NOTIFICATION_PAGINATION_MODEL?> fetchFavorites(int page) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            "fetching notification error ${response.statusCode}: ${json.decode(response.body)["message"]}");
        return null;
      }
    } catch (error) {
  ...
  Future<String> markAsRead(int id) async {
    String apiUrl =
        '${Server.baseUrl}/api/notification/read/$id'; // Đường link API
    String? authToken = AppData.token; // Token xác thực của người dùng
  ...
  Future<String> markAllAsRead() async {
    String apiUrl =
        '${Server.baseUrl}/api/notification/read-all'; // Đường link API
    String? authToken = AppData.token; // Token xác thực của người dùng
  ...
  Future<int?> getUnreadCount() async {
    String apiUrl =
        '${Server.baseUrl}/api/notification/unread/count'; // Đường link API
    String? authToken = AppData.token; // Token xác thực của người dùng

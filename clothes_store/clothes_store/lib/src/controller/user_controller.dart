...
class UserController {
  // đăng ký (POST)
  // http://localhost:5000/api/auth/register?fullname=test flutter account&email=ngduythong1412@gmail.com&password=01412hK2004&phone=0782485283&gender=male
  ...
  Future<SignUpResponse> login(String email, String password) async {
    ...
    if (response.statusCode == 200) {
      ...
      AppData.token = token;
      return SignUpResponse(userModel: newUser, token: token, errorMessage: "");
    } else {
      Map<String, dynamic> jsonResponse = json.decode(response.body);
      return SignUpResponse(
          userModel: null, errorMessage: jsonResponse['message']);
  ...
  Future<ReturnMessager> resetPassword(
    ...
    final response = await http.post(Uri.parse(url),
        headers: headers,
        body:
            json.encode({'email': email, 'otp': otp, 'newPassword': password}));

  ...
  Future<ReturnMessager> addToFavorites(String sku, String note) async {
    ...
    if (authToken != null) {
      ...
      try {
        ...
        return ReturnMessager(
            messager: "Có lỗi xảy ra khi thêm vào yêu thích", statusCode: 0);
      }
    } else {
      print('không tìm thấy token');
      return ReturnMessager(
          messager: "Vui lòng đăng nhập để thêm vào yêu thích", statusCode: 0);
    }
  }
  ...
  Future<ReturnMessager> updateFavoriteNote(int favoriteID, String note) async {
    ...
    if (authToken != null) {
      try {
        ...
        return ReturnMessager(
            messager: "Có lỗi xảy ra khi cập nhật ghi chú", statusCode: 0);
      }
    } else {
      print('không tìm thấy token');
      return ReturnMessager(
